...
void DetCon::DefineDimensions()
{
	...
	// PMT dimensions
	m_PMTD =   46. * mm; // Cathod diameter
	m_PMTT =  100. * mm; // Nominal length
}
...
G4VPhysicalVolume* DetCon::Construct()
{
	...
	// Scintillator
	m_SciSolid1 = new G4Box("SciSolid1", m_SciX / 2., m_SciY / 2., m_SciZ / 2.);
	m_SciSolid2 = new G4Tubs("SciSolid2", 0., m_PMTD / 2., m_PMTT / 2., 0. * degree, 360. * degree);
	m_SciSolid = new G4UnionSolid("SciSolid", m_SciSolid1, m_SciSolid2, 0, G4ThreeVector(0., 0., m_SciZ / 2. + m_PMTT / 2.));
	...
	m_PMTLV = new G4LogicalVolume(m_PMTSolid, m_SciMat, "PMTLV");
	m_PMTLV -> SetVisAttributes(new G4VisAttributes(G4Colour::Gray()));
	m_PMTPV = new G4PVPlacement(0, G4ThreeVector(0., 0., m_SciZ / 2. + m_PMTT / 2.), m_PMTLV, "PMTPV", m_SciLV, false, 0);
	...
	std::vector<G4double> ephoton = { 2.480 * eV, 3.139 * eV };
